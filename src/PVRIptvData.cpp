...
#define GROUP_NAME_MARKER       "#EXTGRP"
...
bool PVRIptvData::LoadPlayList(void)
{
  ...
  while(std::getline(stream, strLine))
  {
    ...
    // There is a code that exctract a group name from the edem.tv playlist
    // it parses #EXTGRP line
    if (StringUtils::Left(strLine, (int)strlen(GROUP_NAME_MARKER)) == GROUP_NAME_MARKER)
    {
      XBMC->Log(LOG_DEBUG, "Found Group line: '%s'", strLine.c_str());
      ...
      if (iColon >= 0)
      {
       std::string strGroupLine = StringUtils::Mid(strLine, ++iColon);
       XBMC->Log(LOG_DEBUG, "strGroupLine is '%s'", strGroupLine.c_str());
       //strGroupName  = ReadMarkerValue(strGroupLine, GROUP_NAME_MARKER); 
       strGroupName = strGroupLine;
       XBMC->Log(LOG_DEBUG, "Group is '%s'", strGroupName.c_str());
       if (!strGroupName.empty())
        {
          std::stringstream streamGroups(strGroupName);   
